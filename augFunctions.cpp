...
vector<int> genInterestList(University* uni){
  ...
  for (InterestProbabilities ip : uni->interestProbabilities){
    if(rand()%100<(ip.probability)*100){
...
vector<Node*> getPeopleWithSameInterest(Node* n){
  ...
  for (int i : n->interests){
    similarInterestPeople = setUnion(similarInterestPeople,n->graph->interestList[i]); 
  }
...
vector<Node*> getPeopleInSameCourse(Node* n){
  ...
  for (Course* c: n->currentCourses){
    allCourseStudents = setUnion(allCourseStudents,c->enrolledStudents);
    if(n->ifStudent){
      removeSelf(&allCourseStudents,c->currentFaculty);
...
int binarySearch(vector<Node*> *v,Node* n,int start,int end){
  if(start>=end){
    return -1;
  }
  else if(v->at((start+end)/2)->nodeID == n->nodeID){
    return (start+end)/2;
  }
  else if(v->at((start+end)/2)->nodeID > n->nodeID){
    return binarySearch(v,n,start,(start+end)/2);
  }
  else{
    return binarySearch(v,n,(start+end)/2,end);
  }

}

bool ifInList(vector<Node*> *v,Node* n){
...
void removeSelf(vector<Node*> *v,Node* n){
 for (int i=0;i<v->size();i++){
  ...
  return;
  // int k = binarySearch(v,n,0,v->size());
  // if(k==-1){return;}
  // else{
  //    v->erase(v->begin()+k);
  //    return;
